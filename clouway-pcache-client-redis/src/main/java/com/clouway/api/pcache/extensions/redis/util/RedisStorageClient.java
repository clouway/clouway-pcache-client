...
//            _log.debug(format("Getting key from Redis (key=%s)", key));
    ...
    public List<byte[]> getAll(final List<String> keys) {

        Callable<List<byte[]>> callable = new RedisCommandCallable<List<byte[]>>() {
            @Override
            protected List<byte[]> execute(BinaryJedis jedis) throws Exception {
                List<byte[]> collect = keys.stream().map(RedisStorageClient::keyBytes).collect(Collectors.toList());
                return jedis.mget(collect.toArray(new byte[][]{}));
            }
        };
    ...
    public Future<Boolean> putAll(List<byte[]> keyvalues) {
        return _executor.submit(new RedisCommandCallable<Boolean>() {
            @Override
            protected Boolean execute(BinaryJedis jedis) throws Exception {
                return !jedis.mset(keyvalues.toArray(new byte[][]{})).isEmpty();
            }
        });
    }
...
//            _log.debug(format("Deleting key in Redis (key=%s)", key));
    ...
    public void flushAll() {
        try {
            new RedisCommandCallable<Boolean>() {
                      @Override
                      protected Boolean execute(BinaryJedis jedis) throws Exception {
                          return !jedis.flushAll().isEmpty();
                      }
                  }.call();
